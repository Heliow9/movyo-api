...
function resolveSqlField(fields, key) {
  if (key === '_id' || key === 'id') return { column: 'id', field: { type: 'VARCHAR(24)', kind: 'id' } };
  if (key === 'createdAt' || key === 'created_at') return { column: 'created_at', field: { type: 'DATETIME', kind: 'date' } };
  if (key === 'updatedAt' || key === 'updated_at') return { column: 'updated_at', field: { type: 'DATETIME', kind: 'date' } };
  if (String(key).includes('.')) return null;
  const field = fields[key];
  return field ? { column: field.column || key, field } : null;
}

function compileSqlFilter(filter, fields) {
  const params = [];

  const compileGroup = (value) => {
    ...
    for (const [key, condition] of Object.entries(value)) {
      if (key === '$or' || key === '$and') {
        ...
        const children = condition.map(compileGroup);
        if (children.some((child) => child === null)) return null;
        const joiner = key === '$or' ? ' OR ' : ' AND ';
        clauses.push(`(${children.join(joiner)})`);
        continue;
      }

      const resolved = resolveSqlField(fields, key);
      if (!resolved) return null;
      const column = `\`${resolved.column}\``;
      const field = resolved.field;

      if (condition === undefined || Array.isArray(condition)) return null;
      if (condition === null) {
      ...
      if (condition instanceof Date || typeof condition !== 'object') {
        clauses.push(`${column} = ?`);
        params.push(serialize(condition, field));
        continue;
      }

      const operators = Object.keys(condition);
      if (!operators.length || operators.some((operator) => !['$in', '$ne', '$gte', '$lte'].includes(operator))) {
        if (isJson(field) && !operators.some((operator) => operator.startsWith('$'))) {
          clauses.push(`${column} = ?`);
          params.push(serialize(condition, field));
          continue;
        }
      ...
      if (condition.$in !== undefined) {
        if (!Array.isArray(condition.$in)) return null;
        if (!condition.$in.length) {
          clauses.push('0 = 1');
        } else {
          clauses.push(`${column} IN (${condition.$in.map(() => '?').join(', ')})`);
          params.push(...condition.$in.map((item) => serialize(item, field)));
        }
      }
      if (condition.$ne !== undefined) {
        if (condition.$ne === null) clauses.push(`${column} IS NOT NULL`);
        else {
          clauses.push(`(${column} <> ? OR ${column} IS NULL)`);
          params.push(serialize(condition.$ne, field));
        }
      }
      if (condition.$gte !== undefined) {
        clauses.push(`${column} >= ?`);
        params.push(serialize(condition.$gte, field));
      }
      if (condition.$lte !== undefined) {
        clauses.push(`${column} <= ?`);
        params.push(serialize(condition.$lte, field));
      }
    }
  ...
  const where = compileGroup(filter || {});
  return where === null ? null : { where, params };
}

function compileSqlSort(sortSpec, fields) {
  if (!sortSpec) return '';
  const spec = typeof sortSpec === 'string'
    ? Object.fromEntries(sortSpec.split(/\s+/).filter(Boolean).map((key) => [key.replace(/^-/, ''), key.startsWith('-') ? -1 : 1]))
    : sortSpec;
  const parts = [];
  for (const [key, direction] of Object.entries(spec || {})) {
    const resolved = resolveSqlField(fields, key);
    if (!resolved) return null;
    parts.push(`\`${resolved.column}\` ${Number(direction) < 0 ? 'DESC' : 'ASC'}`);
  }
  return parts.length ? ` ORDER BY ${parts.join(', ')}` : '';
}

class Query {
  constructor(executor) { this.executor = executor; this._select=null; this._lean=false; this._populate=[]; this._sort=null; this._skip=0; this._limit=null; this._sqlApplied=false; }
  ...
  session(){ return this; }
  async exec(){
    let res = await this.executor(this);
...
function defineModel(name, definition) {
  ...
  class Model {
    ...
    static async _updateById(id, update, options={}){
      ...
      row.updated_at = formatDateTimeForMysql(updatedAt);
      ...
      const persistedData = { ...current, _id: normalizeId(id), id: normalizeId(id), updatedAt };
      for (const [prop, field] of Object.entries(fields)) {
        if (prop === '_id') continue;
        const column = field.column || prop;
        if (Object.prototype.hasOwnProperty.call(row, column)) {
          setByPath(persistedData, prop, deserialize(row[column], field));
        }
      }
      const persisted = new Model(persistedData);
    ...
    static find(filter={}){ return new Query(async(query)=>{
      await ensureTable();
      const compiled = compileSqlFilter(filter, fields);
      const orderBy = compileSqlSort(query?._sort, fields);
      const requestedLimit = query?._limit;
      const canPushPagination = requestedLimit == null || (
        Number.isFinite(Number(requestedLimit)) && Number(requestedLimit) >= 0
      );
      if (compiled && orderBy !== null && canPushPagination) {
        let pagination = '';
        const params = [...compiled.params];
        const hasLimit = query?._limit != null && Number.isFinite(Number(query._limit));
        const hasSkip = Number(query?._skip || 0) > 0;
        if (hasLimit) {
          pagination = ' LIMIT ?';
          params.push(Math.max(0, Math.trunc(Number(query._limit))));
          if (hasSkip) {
            pagination += ' OFFSET ?';
            params.push(Math.trunc(Number(query._skip)));
          }
        } else if (hasSkip) {
          pagination = ' LIMIT 18446744073709551615 OFFSET ?';
          params.push(Math.trunc(Number(query._skip)));
        }
        const [rows] = await queryWithRetry(
          `SELECT * FROM \`${table}\` WHERE ${compiled.where}${orderBy}${pagination}`,
          params
        );
        query._sqlApplied = true;
    ...
    static async findOneAndUpdate(filter, update, opts={}){ const doc = await Model.findOne(filter).lean(); if (!doc) return null; return Model.findByIdAndUpdate(doc._id, update, { ...opts, previous: doc }); }
    static async updateOne(filter, update){ const doc = await Model.findOne(filter).lean(); if (!doc) return { modifiedCount:0 }; await Model.findByIdAndUpdate(doc._id, update, { previous: doc }); return { modifiedCount:1 }; }
    static async updateMany(filter, update){ const docs = await Model.find(filter).lean(); for (const d of docs) await Model.findByIdAndUpdate(d._id, update, { previous: d }); return { modifiedCount: docs.length }; }
...
module.exports = {
  ...
  syncAllModels,
  modelRegistry,
  _test: { compileSqlFilter, compileSqlSort, resolveSqlField },
};
