...
function defineModel(name, definition) {
  ...
  class Model {
    ...
    static _toRow(data, partial=false){
      const row = {};
      const shouldReplaceAliasValue = (oldValue, newValue) => {
        if (newValue === undefined) return false;
        if (oldValue === undefined) return true;
        if (oldValue === null || oldValue === '') return newValue !== null && newValue !== '';
        if (newValue === null || newValue === '') return false;

        const oldNum = typeof oldValue === 'number' ? oldValue : Number(String(oldValue).replace(',', '.'));
        const newNum = typeof newValue === 'number' ? newValue : Number(String(newValue).replace(',', '.'));
        const oldIsNum = Number.isFinite(oldNum);
        const newIsNum = Number.isFinite(newNum);

        // Campos aliases usam a mesma coluna física. Ex.: Pedido.total e Pedido.valorTotal
        // apontam para `total`. Como defaults podem deixar um alias em 0, nunca deixe
        // esse 0 sobrescrever o outro alias com valor real.
      ...
      for (const [prop, f] of Object.entries(fields)) {
        ...
        const col = f.column || prop;
        const serialized = serialize(value, f);
        if (shouldReplaceAliasValue(row[col], serialized)) row[col] = serialized;
      }
      return row;
    }
    static _fromRow(row){
