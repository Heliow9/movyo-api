...
function applyUpdatePath(out, current, key, value) {
  ...
  // Compatibilidade Mongo -> MySQL JSON:
  // updates como { $set: { 'statusBot.ligado': true } } precisam regravar
  // a coluna JSON statusBot inteira, senão o adapter ignora o campo aninhado.
  const [top, ...rest] = path.split('.');
  const base = out[top] !== undefined
    ? clonePlain(out[top])
    : clonePlain(getByPath(current, top) || {});
  const obj = base && typeof base === 'object' && !Array.isArray(base) ? base : {};
  setByPath(obj, rest.join('.'), value);
  out[top] = obj;
}
...
function flattenUpdate(update, current={}) {
  ...
  if (update.$set) {
    Object.entries(update.$set).forEach(([k, v]) => applyUpdatePath(out, current, k, v));
  }

  if (update.$inc) {
    Object.entries(update.$inc).forEach(([k, v]) => {
      const next = Number(getByPath(current, k) || 0) + Number(v || 0);
      applyUpdatePath(out, current, k, next);
    });
  }

  if (update.$unset) {
    Object.keys(update.$unset).forEach((k) => applyUpdatePath(out, current, k, null));
  }

  Object.keys(update).forEach((k) => {
    if (!k.startsWith('$')) applyUpdatePath(out, current, k, update[k]);
  });
